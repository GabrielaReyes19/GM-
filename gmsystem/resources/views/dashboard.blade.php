...
var oTableOptions = {
    ...
    "ajax":{
    "url": "{{ url('api') }}.{{ $nameModule }}/"+$("#doc").val(),
    "data": function ( d ) {
        return $.extend( {}, d, {
            "buscar": "0"
        });
    ...
    "columnDefs": 
    [
        {"targets": 0, 'data': 'pk_cli_id', 'name': 'pk_cli_id'},
        {"targets": 1, 'data': 'nombre_cliente', 'name':'nombre_cliente'},
        {"targets": 2, 'data': 'c_num_doc', 'name':'c_num_doc'},
        {"targets": 3, 'data': 'c_raz', 'name':'c_raz'},
        {"targets": 4, 'data': 'c_dir', 'name':'c_dir'},
        {"targets": 5, 'data': 'c_dir', 'name':'c_dir'},
        {"targets": 6, 'data': 'action', 'name': 'action', 'orderable': false, 'searchable': false}
...
function renderUpdate(id){
    ...
    $.ajax({
        ...
        success:function(response){
            if (response != "false") {
                $('#content-modal-{{ $nameModule }}').html(response);
                $.growl({ title: "OK:", message: "Se carg&oacute; cliente<br/>ID: " + id, size: 'large' }); 
            }else{
                alert("No se cre&oacute; cliente"+response.error);
            }
        },
            ...
            success : function(datos_json) {             
                if (datos_json.success != false) {
                    if(datos_json.sesActiva != "0"){
                        $.growl.notice({
                            title: "Mensaje:",
                            message: "Se elimin&oacute; cliente:<br />ID: "+id,
                            size: 'large'
                        });
