...
<div class="modal-body">
    <form id="modal-{{ $nameModule }}-form">
        ...
        <div class="table-light">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr>
                        <td>
                            {{ $ComprobanteCabecera[0]->f_hor_fac }}
                            <input type="hidden" name="id" value="{{$ComprobanteCabecera[0]->pk_cpb_id}}">
                            <input type="hidden" name="numDoc" value="{{$ComprobanteCabecera[0]->c_num}}">
                            <input type="hidden" name="serBol" value="{{$ComprobanteCabecera[0]->c_mod_num_ser}}">
                            <input type="hidden" name="docBol" value="{{$ComprobanteCabecera[0]->c_mod_doc}}">
                            <input type="hidden" name="num" value="{{$ComprobanteCabecera[0]->rownum}}">
                            <input type="hidden" name="serie" value="{{$ComprobanteCabecera[0]->c_num_ser}}">
                            <input type="hidden" name="correlativo" value="{{$ComprobanteCabecera[0]->c_doc}}">
                            <input type="hidden" name="subTotal" value="{{$ComprobanteCabecera[0]->n_sub_tot}}">
                            <input type="hidden" name="igv" value="{{$ComprobanteCabecera[0]->n_igv}}">
                            <input type="hidden" name="total" value="{{$ComprobanteCabecera[0]->n_tot}}">
                            <input type="hidden" name="docCliente" value="{{$ComprobanteCabecera[0]->c_num_doc}}">

                            <input type="hidden" name="idCliente" value="{{$ComprobanteCabecera[0]->fk_cli_id}}">
                            <input type="hidden" name="idTipDocVenta" value="{{$ComprobanteCabecera[0]->fk_tip_doc_ven_id}}">
                        </td>
        ...
        <div class="panel-footer text-right tab-content-padding">
            <input type="hidden" name="id" value="{{ $ComprobanteCabecera[0]->pk_cpb_id }}">
            <input type="hidden" name="num" value="{{ $correlativo }}"/>
